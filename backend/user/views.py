...
from .models import Role, User, Verification
from .my_logger import logger
from .permissions import IsCreationOrIsAuthenticated
from .serializers import UserResponseSerializer, UserSerializer, VerificationSerializer
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            if Verification.objects.filter(email=request.data[EMAIL]):
                ...
                return Response(
                    {DETAIL: VERIFY_MAIL}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )
        except Exception as e:
            raise DataNotExist(e.__str__())
    ...
    def delete(request, pk):
        ...
        try:
            ...
            if not user.is_deleted:
                ...
                return Response(
                    {DETAIL: USER_NOT_EXIST}, status=status.HTTP_404_NOT_FOUND
                )
        except Exception as e:
            raise DataNotExist(e.__str__())
    ...
    def post(request):
        try:
            ...
            if otp == generate_otp(email):
                ...
                return Response(
                    {DETAIL: OTP_VERIFICATION_SUCCESS}, status=status.HTTP_200_OK
                )
            else:
                logger.warn(OTP_DOES_NOT_MATCH)
                return Response(
                    {DETAIL: OTP_DOES_NOT_MATCH}, status=status.HTTP_400_BAD_REQUEST
                )
        except Exception as e:
            raise DataNotExist(e.__str__())
