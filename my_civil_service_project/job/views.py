...
from my_exceptions import DataNotExist, DataAlreadyExist
...
class JobViewSets(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        """
        try:
            request.data[WORK_TYPE] = WorkType.objects.get(name=request.data[WORK_TYPE]).id
            address = Address.objects.get(is_deleted=False, id=request.data[ADDRESS])
            job = Job.objects.filter(work_date=request.data[WORK_DATE], requestor=request.data['requestor'])
...
class ApplyOffer(APIView):
    ...
    def post(request):
        ...
        try:
            ...
            return Response({DETAIL: 'Successfully Applied', 'status': 200, 'timestamp': datetime.datetime.now()})
        except Exception as e:
            return Response({DETAIL: e.__str__(), 'status': 400, 'timestamp': datetime.datetime.now()})
    ...
    def get(request):
        ...
        try:
            if job_request[request.data['job_id']]:
                ...
                return Response({DETAIL: 'No one applied', 'status': 200, 'timestamp': datetime.datetime.now()})
        except Exception as e:
            return Response({DETAIL: e.__str__(), 'status': 400, 'timestamp': datetime.datetime.now()})
...
class RejectOffer(APIView):
    ...
    def put(request, **kwargs):
        ...
        try:
            ...
            return Response({DETAIL: 'Rejected successfully'}, status=status.HTTP_200_OK)
        except Exception as e:
            raise DataNotExist(e.__str__())
    ...
    def delete(request, pk):
        ...
        try:
            ...
            if not job.is_deleted:
                ...
                return Response({DETAIL: JOB_NOT_EXIST}, status=status.HTTP_400_BAD_REQUEST)
        except Exception as e:
            raise DataNotExist(e.__str__())
        ...
        try:
            if request.data:
                ...
                if WORK_TYPE in request.data and len(request.data) == 1:
                    ...
                    days = request.data[DAYS]
                    pay = request.data[PAY]
                    jobs = Job.objects.filter(is_deleted=False, working_days=days, work_pay=pay)

                elif WORK_TYPE and WORK_DATE in request.data and len(request.data) == 2:
                    date = request.data[WORK_DATE]
                    pay = request.data[PAY]
                    jobs = Job.objects.filter(is_deleted=False, work_date=date, work_pay=pay)

                elif WORK_DATE and DAYS in request.data and len(request.data) == 2:
                    date = request.data[WORK_DATE]
                    days = request.data[DAYS]
                    jobs = Job.objects.filter(is_deleted=False, work_date=date, working_days=days)

                elif DAYS and WORK_DATE and PAY in request.data and len(request.data) == 3:
                    date = request.data[WORK_DATE]
                    pay = request.data[PAY]
                    days = request.data[DAYS]
                    jobs = Job.objects.filter(is_deleted=False, work_date=date, working_days=days, work_pay=pay)
