    ...
    def delete(request, pk):
        ...
        try:
            ...
            if not shop.is_deleted:
                ...
                return Response({DETAIL: SHOP_NOT_EXIST}, status=status.HTTP_404_NOT_FOUND)
        except Exception as e:
            raise DataNotExist(e.__str__())
        ...
        try:
            if request.data:
                if CITY and TYPE in request.data and len(request.data) == 2:
                    ...
                    for i in address:
                        shop = shop_type.shops.filter(id=i.__dict__[MODULE_FIELD_ID], is_deleted=False)
                        if shop:
                    ...
                    result = []
                    for i in address:
                        shop = Shop.objects.filter(name=name, id=i.__dict__[MODULE_FIELD_ID], is_deleted=False)
                        if shop:
                            shop_serializer = ShopResponseSerializer(shop, many=True)
                            result.append(shop_serializer.data)
                    if not result:
