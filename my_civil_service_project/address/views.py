...
from .constants import INVALID_VALUE, INVALID_KEY
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            if type == HOME_ADDRESS:
                ...
                created_by = User.objects.get(is_deleted=False, id=request.data[MODULE_FIELD_ID])
                request.data[CREATED_BY] = created_by.id
                request.data[UPDATED_BY] = created_by.id
            elif type == WORK_ADDRESS:
                request.data[MODULE] = 2
                created_by = Profession.objects.get(is_deleted=False, id=request.data[MODULE_FIELD_ID])
                request.data[CREATED_BY] = created_by.address.id
                request.data[UPDATED_BY] = created_by.address.id
            elif type == SHOP_ADDRESS:
                created_by = Shop.objects.get(is_deleted=False, id=request.data[MODULE_FIELD_ID])
                request.data[CREATED_BY] = created_by.address.id
                request.data[UPDATED_BY] = created_by.address.id
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            ...
            if address_type == HOME_ADDRESS:
                ...
                request.data[MODULE] = 2
                try:
                    Profession.objects.get(is_deleted=False, id=request.data[MODULE_FIELD_ID])
                except Exception as e:
                    raise DataNotExist(e.__str__())
...
def delete_address(request, pk):
    ...
    try:
        ...
        if not address.is_deleted:
            ...
            return Response({DETAIL: ADDRESS_NOT_FOUND}, status=status.HTTP_400_BAD_REQUEST)
    except Exception as e:
        raise DataNotExist(e.__str__())
